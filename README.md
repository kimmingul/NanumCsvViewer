...
- (SAS 값 라벨은 외부 `.sas7bcat` 카탈로그에 있고 순수 관리형 .NET 리더가 없어 지원하지 않습니다 — 문자 컬럼은 저장된 텍스트를 그대로 표시.)

### ✨ v1.12.0 신규 — SPSS·SAS 선언 타입 매칭 + 타입 확장

- **선언된 변수 타입을 자동 매칭** — SPSS·SAS 파일에 지정된 컬럼 타입(통화·퍼센트·날짜·범주·순서형)을 추론 대신 **파일 명시대로** 표시합니다. 예: 값 라벨이 붙은 코드 컬럼은 범주형, `DOLLAR`/`PCT` 포맷은 통화/퍼센트로 인식하고, 숫자로 선언된 `id`는 Identifier로 오인하지 않습니다.
- **타입 4종 추가** — `Currency`·`Percent`·`Ordinal`·`Scientific`. 통화는 **₩ $ ¥/元 €/£** 기호를 인식하며, 통화·퍼센트 열은 그리드에 **기호(₩1,000·25%)를 함께 표시**합니다(정렬·통계·필터는 밑값 숫자로 동작).
- CSV에서도 `₩1,000`·`25%`·`1.2e3` 같은 값을 통화·퍼센트·지수로 자동 추론합니다.

### 🔧 v1.11.1 — 필드 라벨 툴바 버튼

- **필드 라벨 표시 전환 툴바 버튼** 추가 — 타입 배지 버튼 왼쪽에 위치하며 보기 메뉴 토글과 동기화됩니다. SPSS·SAS일 때만 활성(그 외 파일에선 회색). 배지 버튼과 구별되는 초록 태그 아이콘.

### ✨ v1.11.0 신규 — SPSS·SAS 필드 라벨 표시

- **필드 라벨 토글** — **보기 ▸ 필드 라벨 표시**로 SPSS·SAS 파일의 **변수 라벨을 헤더로**, SPSS는 **값 라벨로 코드를 치환**(예: `1`→`남`)해 봅니다. 끄면 원값·변수명으로 돌아갑니다(기본값). 선택은 저장되어 다음 열기의 기본값이 됩니다.
- SAS는 파일에 값 라벨이 없어(포맷 카탈로그는 별도) **변수 라벨(헤더)만** 적용됩니다. SPSS 날짜 변수는 원값 모드에서도 날짜로 표시됩니다.
- (R `.rds/.RData`는 순수 관리형 리더가 없어 별도 검토 예정입니다.)

### ✨ v1.10.0 신규 — SPSS(.sav) 열기

- **SPSS 데이터 파일(.sav) 지원** — 파일을 열면 데이터셋을 자동으로 읽어 CSV처럼 필터·정렬·타입 배지·패싯·내보내기를 그대로 사용합니다. 엑셀·SAS와 동일하게 임시 CSV로 변환해 기존 엔진에 얹으며, [Curiosity.SPSS](https://www.nuget.org/packages/Curiosity.SPSS)(MIT, 순수 관리형)를 사용합니다.
- 현재는 **원값(코드)** 을 표시해 타입 추론을 보존합니다. Value Label 펼치기·Variable Label 노출·날짜 변수 디코딩은 후속(Phase 2)으로 계획되어 있습니다.

### 🐞 v1.9.1 — 도움말 갱신·버전 정보 연동

- **사용법(F1)·정보 창** 내용을 최신 기능(엑셀·SAS·SPSS 열기, 타입 인식 필터, 칩 바, 패싯 패널)으로 갱신.
